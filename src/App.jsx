...
const FEATURES = [
  { icon: <Tag className="h-6 w-6" />, fr: "Meilleures liquidations vérifiées", en: "Verified best clearances" },
  { icon: <Bell className="h-6 w-6" />, fr: "Alertes par courriel / SKU", en: "Email / SKU alerts" },
  { icon: <ShoppingCart className="h-6 w-6" />, fr: "Par magasin & disponibilité", en: "By store & availability" },
  { icon: <Globe2 className="h-6 w-6" />, fr: "Canada d’abord, USA ensuite", en: "Canada first, USA next" },
];
  ...
  useEffect(() => {
    const go = async () => {
      ...
      try {
        setLoadingEbay(true);
        setErrorEbay("");
        const r = await fetch(`/api/ebay?q=electronics&limit=8&env=PROD`);
        const j = await r.json();
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-emerald-50 to-white text-slate-800">
      ...
      <section id="deals" className="mx-auto max-w-7xl px-4 py-8">
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-6">
          {storeValue === "ebay" ? (
            loadingEbay ? (
              ...
              ebayItems.map((it) => (
                <article key={it.id} className="group rounded-2xl border bg-white overflow-hidden hover:shadow-md transition">
                  ...
                  <div className="p-4">
                    ...
                    <div className="mt-2 flex items-baseline gap-2">
                      {it.price ? (<span className="text-lg font-bold">{fmt(convert(it.price, it.currency || "USD"))} {currency}</span>) : (<span className="text-slate-500 text-sm">Prix N/A</span>)}
                    </div>
                    <a href={it.link} target="_blank" rel="noreferrer" className="mt-2 inline-block text-sm text-emerald-700 hover:underline">Voir</a>
                  </div>
                </article>
              ))
            ) : (
            ...
            DEALS.filter(d => storeValue === "all" || d.store.toLowerCase() === storeValue).map((d) => (
              <article key={d.id} className="group rounded-2xl border bg-white overflow-hidden hover:shadow-md transition">
                <div className="relative">
        ...
        <p className="mt-3 text-sm text-slate-500">* Les quantités eBay proviennent du champ « estimatedAvailableQuantity » (si fourni).</p>
      </section>
